-- Clean up existing policies (Adding missing DROPs)
DROP POLICY IF EXISTS "Allow full access to authenticated users" ON public.profiles;
DROP POLICY IF EXISTS "Public profiles are viewable by everyone" ON public.profiles;
DROP POLICY IF EXISTS "Admins can update any profile" ON public.profiles;
DROP POLICY IF EXISTS "Users can update own profile" ON public.profiles; -- Added this missing DROP
...
-- 5. Super Admin Function (Added missing DROP)
